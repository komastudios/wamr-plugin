...
#include "../../../wamr/product-mini/platforms/common/libc_wasi.c"
#endif

#include "../../../wamr/product-mini/platforms/common/wasm_proposal.c"
...
print_help(void)
{
    ...
    printf("                           are dynamic link library (.dll) files, for example:\n");
    printf("                             --native-lib=test1.dll --native-lib=test2.dll\n");
...
load_native_lib(const char *name)
{
    ...
    lib->handle = LoadLibraryExA(name, NULL, LOAD_WITH_ALTERED_SEARCH_PATH);
    if (lib->handle == NULL) {
        lib->handle = LoadLibraryA(name);
    }
    if (lib->handle == NULL) {
        load_error = GetLastError();
        LOG_WARNING("warning: failed to load native library %s. error code %lu",
                    name, (unsigned long)load_error);
        goto fail;
    }

    lib->init_native_lib =
        (int (*)(void))GetProcAddress(lib->handle, "init_native_lib");
    lib->get_native_lib = (uint32(*)(char **, NativeSymbol **))GetProcAddress(
        lib->handle, "get_native_lib");
    lib->deinit_native_lib =
        (void (*)(void))GetProcAddress(lib->handle, "deinit_native_lib");
...
main(int argc, char *argv[])
{
    ...
    int instance_port = 0;
#endif
    const char *native_lib_list[MAX_NATIVE_LIBS] = { NULL };
    uint32 native_lib_count = 0;
    struct native_lib *native_lib_loaded_list[MAX_NATIVE_LIBS];
